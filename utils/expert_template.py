...
def render_youtube_section(query: str, limit: int = 12, per_page: int = 4,
                           sort: str = "views", auto_load: bool = False) -> list[dict]:
    ...
    _youtube_section_fragment(query, limit, per_page, sort, _tl, _yt_gate, auto_load)
...
def render_expert_page(
    ...
    _desc_map = {
        ...
        "스포츠": "KBO·EPL·NBA 등 국내외 스포츠 경기·결과를 분석합니다.",
        "연예/엔터": "K-POP·드라마·영화·예능 최신 동향을 제공합니다.",
        "자동차": "신차·중고차·전기차·자동차 보험 트렌드를 분석합니다.",
        "보험/연금": "국민연금·건강보험·실손보험 정책과 비교 정보를 제공합니다.",
        "환경/에너지": "미세먼지·기후변화·신재생에너지 동향을 모니터링합니다.",
        "암호화폐": "비트코인·이더리움·가상자산 시세와 규제 동향을 분석합니다.",
    }
